...
const CYCLE_OPTIONS:Cycle[]=["Open","Closed"];
...
function cycleOf(d:Drive):Cycle{return d.cooling==="Open"?"Open":"Closed"}
...
const SMALL_UNIT_PREFIXES=[{factor:1e-1,symbol:"d"},{factor:1e-2,symbol:"c"},{factor:1e-3,symbol:"m"},{factor:1e-6,symbol:"µ"},{factor:1e-9,symbol:"n"},{factor:1e-12,symbol:"p"},{factor:1e-15,symbol:"f"},{factor:1e-18,symbol:"a"},{factor:1e-21,symbol:"z"},{factor:1e-24,symbol:"y"}] as const;
function absoluteMaterialUnits(value:number){const amount=Math.abs(value),prefix=SMALL_UNIT_PREFIXES.find(item=>amount>=item.factor)??SMALL_UNIT_PREFIXES[SMALL_UNIT_PREFIXES.length-1];return `${new Intl.NumberFormat("en",{maximumSignificantDigits:3}).format(value/prefix.factor)} ${prefix.symbol} units`}
function compositionOf(d:Drive){const entries=Object.entries(d.perTankPropellantMaterials??{}).filter(([,value])=>value>0);return entries.length?entries.map(([name,value])=>`${new Intl.NumberFormat("en",{maximumFractionDigits:1}).format(value*100)}% ${readable(name)}${name.toLowerCase()==="antimatter"?` (${absoluteMaterialUnits(value)})`:""}`).join(" · "):"No resource mix listed"}
function majorityCompositionSearch(d:Drive){return Object.entries(d.perTankPropellantMaterials??{}).filter(([,value])=>value>=.5).flatMap(([name])=>[name,readable(name)]).join(" ")}
...
function formatEfficiency(d:Drive){return `${new Intl.NumberFormat("en",{style:"percent",maximumFractionDigits:1}).format(d.efficiency)}`}
function formatCoolingCycle(d:Drive){return d.cooling==="Open"?"Open cycle":d.cooling==="Calc"?"Closed cycle (calculated)":"Closed cycle"}
...
function CycleFilter({excluded,onToggle,onAll,onNone}:{excluded:Set<Cycle>;onToggle:(value:Cycle)=>void;onAll:()=>void;onNone:()=>void}){
  const included=CYCLE_OPTIONS.length-excluded.size;
  return <details className="filter-dropdown cycle-dropdown"><summary>{included===CYCLE_OPTIONS.length?"All cycles":`${included} of ${CYCLE_OPTIONS.length} cycles`}<span aria-hidden="true">⌄</span></summary><div className="filter-menu"><div className="filter-actions"><button type="button" onClick={onAll}>Select all</button><button type="button" onClick={onNone}>None</button></div>{CYCLE_OPTIONS.map(option=><label className="filter-option" key={option}><input type="checkbox" checked={!excluded.has(option)} onChange={()=>onToggle(option)}/><span>{option} cycle</span></label>)}</div></details>
}
...
export function DriveExplorer(){
  ...
  const [excludedCycles,setExcludedCycles]=useState<Set<Cycle>>(()=>new Set());
  ...
  const filtered=useMemo(()=>{
    ...
    return candidates.filter(d=>(chartMode!=="installed"||!excludedCycles.has(cycleOf(d)))&&!excludedPropellants.has(d.propellant)&&!excludedFamilies.has(familyOf(d))&&!excludedSubtypes.has(subtypeKey(d))&&(!n||`${d.friendlyName} ${d.driveClassification} ${d.requiredPowerPlant} ${d.propellant} ${majorityCompositionSearch(d)}`.toLowerCase().includes(n)))
  },[chartMode,drives,excludedCycles,excludedFamilies,excludedPropellants,excludedSubtypes,maxOnly,query]);
  ...
  const toggleCycle=(value:Cycle)=>setExcludedCycles(current=>{const next=new Set(current);if(next.has(value))next.delete(value);else next.add(value);return next});
  ...
  return <main className={`app-shell${readableText?" readable-text":""}`}>
    ...
    <section className="workspace">
      <div className="panel"><div className="panel-head"><h2 className="panel-title">Performance envelope</h2><span className="count">{filtered.length} shown</span></div>
        ...
        {chartMode==="installed"&&<aside className="installed-guidance" role="note" aria-labelledby="installed-guidance-title"><strong id="installed-guidance-title">How to read this view</strong><div className="installed-guidance-copy"><p>Specific thrust includes only the drive, automatically selected reactor, and radiator—not fuel, tanks, armor, or the rest of the ship. Larger ships, particularly heavily armored later-game designs, require proportionally more thrust to achieve the same acceleration in milligees (mg). Open-cycle drives expel heat through their exhaust and are modeled without radiator mass; closed-cycle drives include the selected radiator. “Calculated cooling” drives are grouped as closed-cycle.</p><p>The automatic reactor minimizes installed propulsion mass. High specific thrust favors acceleration, while exhaust velocity—not this value—better represents propellant efficiency and delta-v.</p></div></aside>}
        ...
        {error?<div className="empty-state">{error}</div>:!drives.length?<div className="loading">Loading drive telemetry…</div>:!plotted.length?<div className="empty-state">No drives match the active filters or have a valid installed system.</div>:<div className="chart-wrap" onMouseLeave={closeTooltipSoon}><svg className="chart" viewBox={`0 0 ${chart.width} ${chart.height}`} role="img" aria-label={`${mode.label} scatter plot`}>
          {chartMode==="power"&&<><rect className="zero-power-lane" x={chart.left} y={chart.top} width={Math.max(18,px(powerFloor*10)-chart.left)/2} height={chart.height-chart.top-chart.bottom}/><text className="zero-power-label" x={chart.left+14} y={chart.height-chart.bottom+19}>SELF-POWERED · 0 GW</text></>}
